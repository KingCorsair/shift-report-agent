...
def comparison_table(cur: dict, prior: dict) -> pd.DataFrame:
    """Side-by-side of the headline metrics: this shift, prior shift, change."""
    def change(cur_v, prev_v, unit=""):
        d = cur_v - prev_v
        pct = f" ({d / prev_v * 100:+.1f}%)" if prev_v else ""
        arrow = "▲" if d > 0 else "▼" if d < 0 else "→"
        return f"{arrow} {d:+g}{unit}{pct}"

    rate_d = (cur["defect_rate"] - prior["defect_rate"]) * 100
    rows = [
        ("Units produced", fmt(cur["units"]), fmt(prior["units"]),
         change(cur["units"], prior["units"])),
        ("Downtime", f"{fmt(cur['downtime'])} min", f"{fmt(prior['downtime'])} min",
         change(cur["downtime"], prior["downtime"], " min")),
        ("Defects", fmt(cur["defects"]), fmt(prior["defects"]),
         change(cur["defects"], prior["defects"])),
        ("Defect rate", f"{cur['defect_rate'] * 100:.1f}%",
         f"{prior['defect_rate'] * 100:.1f}%",
         f"{'▲' if rate_d > 0 else '▼' if rate_d < 0 else '→'} {rate_d:+.1f} pts"),
    ]
    return pd.DataFrame(rows, columns=["Metric", "This shift", "Prior shift", "Change"]).set_index("Metric")


def build_comparison_report(cur_id, prior_id, cur, prior, cmp_lines,
                            date_str, period) -> str:
    """Plain-text version of the separate comparison report."""
    rule = "=" * 58
    L = [rule, "SHIFT COMPARISON REPORT", f"{period} · {cur_id}",
         f"vs prior shift · {prior_id}", date_str, rule, ""]
    L.append("IN PLAIN LANGUAGE")
    for t in cmp_lines:
        L.append(f"  - {t.replace('**', '')}")
    L.append("")
    L.append("SIDE BY SIDE")
    L.append(f"  {'Metric':<16} {'This shift':>12} {'Prior shift':>12}")

    def row(label, a, b):
        L.append(f"  {label:<16} {str(a):>12} {str(b):>12}")

    row("Units produced", fmt(cur["units"]), fmt(prior["units"]))
    row("Downtime", f"{fmt(cur['downtime'])} min", f"{fmt(prior['downtime'])} min")
    row("Defects", fmt(cur["defects"]), fmt(prior["defects"]))
    row("Defect rate", f"{cur['defect_rate'] * 100:.1f}%", f"{prior['defect_rate'] * 100:.1f}%")
    L.append("")
    L.append(rule)
    L.append("Generated by Shift Report Agent")
    return "\n".join(L)
...
st.header("How this shift compares")

cmp_lines = comparison_lines(cur, prior)
if not cmp_lines:
    st.info("This log has only one shift, so there's no prior shift to compare against.")
else:
    st.caption(f"{cur_id}  vs prior shift  {prior_id}")

    st.subheader("In plain language")
    st.markdown("\n".join(f"- {line}" for line in cmp_lines))

    st.subheader("Side by side")
    st.table(comparison_table(cur, prior))

    comparison_text = build_comparison_report(
        cur_id, prior_id, cur, prior, cmp_lines, date_str, period,
    )
    st.download_button(
        "Download comparison (.txt)",
        comparison_text,
        file_name=f"shift-comparison-{cur_id}.txt",
        mime="text/plain",
        type="primary",
    )
