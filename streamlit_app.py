...
# ---- Comparison report (a separate report vs the prior shift) ----------------
...
def comparison_lines(cur: dict, prior: dict | None) -> list[str] | None:
    ...
    Bold uses markdown (**) on-screen; the text report strips it. None when
    there is no prior shift."""
    if not prior:
        return None
...
def comparison_table(a: dict, b: dict, a_label: str, b_label: str) -> pd.DataFrame:
    """Side-by-side of the headline metrics for two shifts, with the change
    (shift a relative to shift b). Columns are named after the two shifts."""
    def change(a_v, b_v, unit=""):
        d = a_v - b_v
        pct = f" ({d / b_v * 100:+.1f}%)" if b_v else ""
        arrow = "▲" if d > 0 else "▼" if d < 0 else "→"
        return f"{arrow} {d:+g}{unit}{pct}"

    rate_d = (a["defect_rate"] - b["defect_rate"]) * 100
    rows = [
        ("Units produced", fmt(a["units"]), fmt(b["units"]),
         change(a["units"], b["units"])),
        ("Downtime", f"{fmt(a['downtime'])} min", f"{fmt(b['downtime'])} min",
         change(a["downtime"], b["downtime"], " min")),
        ("Defects", fmt(a["defects"]), fmt(b["defects"]),
         change(a["defects"], b["defects"])),
        ("Defect rate", f"{a['defect_rate'] * 100:.1f}%",
         f"{b['defect_rate'] * 100:.1f}%",
         f"{'▲' if rate_d > 0 else '▼' if rate_d < 0 else '→'} {rate_d:+.1f} pts"),
    ]
    return pd.DataFrame(
        rows, columns=["Metric", a_label, b_label, "Change"]
    ).set_index("Metric")


def build_comparison_report(a_id, b_id, a, b, cmp_lines) -> str:
    """Plain-text version of the separate comparison report (shift a vs b)."""
    rule = "=" * 58
    L = [rule, "SHIFT COMPARISON REPORT", f"{a_id}  vs  {b_id}", rule, ""]
    L.append("IN PLAIN LANGUAGE")
    for t in cmp_lines:
        L.append(f"  - {t.replace('**', '')}")
    L.append("")
    L.append("SIDE BY SIDE")
    w = max(18, len(a_id), len(b_id))
    L.append(f"  {'Metric':<16} {a_id:>{w}} {b_id:>{w}}")

    def row(label, av, bv):
        L.append(f"  {label:<16} {str(av):>{w}} {str(bv):>{w}}")

    row("Units produced", fmt(a["units"]), fmt(b["units"]))
    row("Downtime", f"{fmt(a['downtime'])} min", f"{fmt(b['downtime'])} min")
    row("Defects", fmt(a["defects"]), fmt(b["defects"]))
    row("Defect rate", f"{a['defect_rate'] * 100:.1f}%", f"{b['defect_rate'] * 100:.1f}%")
    L.append("")
    L.append(rule)
    L.append("Generated by Shift Report Agent")
    return "\n".join(L)
...
# Pick any two shifts to compare side by side (defaults to latest vs prior).
st.divider()
st.caption("COMPARISON REPORT")
st.header("Compare two shifts")

if len(shifts) < 2:
    st.info("This log has only one shift, so there's nothing to compare.")
else:
    ca, cb = st.columns(2)
    a_id = ca.selectbox("Compare", shifts, index=len(shifts) - 1, key="cmp_a")
    b_id = cb.selectbox("against", shifts, index=len(shifts) - 2, key="cmp_b")

    if a_id == b_id:
        st.warning("Pick two different shifts to compare.")
    else:
        a_sum = summarize(df[df["shift_id"] == a_id])
        b_sum = summarize(df[df["shift_id"] == b_id])
        cmp_lines = comparison_lines(a_sum, b_sum)

        st.subheader("In plain language")
        st.markdown(f"**{a_id}** compared with **{b_id}**:")
        st.markdown("\n".join(f"- {line}" for line in cmp_lines))

        st.subheader("Side by side")
        st.table(comparison_table(a_sum, b_sum, a_id, b_id))

        comparison_text = build_comparison_report(a_id, b_id, a_sum, b_sum, cmp_lines)
        st.download_button(
            "Download comparison (.txt)",
            comparison_text,
            file_name=f"shift-comparison-{a_id}-vs-{b_id}.txt",
            mime="text/plain",
            type="primary",
        )
        with st.expander("View plain-text comparison report"):
